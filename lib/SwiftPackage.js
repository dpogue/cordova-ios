...
function mergeSwiftPackage (projectConfigEl, platformConfigEl) {
    let iOSPlatformTag = findIosPlatformTag(projectConfigEl);
    if (iOSPlatformTag) {
        let found = false;
        platformConfigEl.findall('platform').forEach(el => {
            el.attrib.package = 'swift';  // 属性追加または上書き
            found = true;
        });
        if (!found) {
            const newPlatform = new et.Element('platform');
            newPlatform.attrib.package = 'swift';
            platformConfigEl.append(newPlatform);
        }
    }
}

function findIosPlatformTag (projectConfigEl) {
    let iOSPlatformTag = null;
    projectConfigEl.findall(`platform[@name="ios"]`).forEach(platformElement => {
        const pkg = platformElement.attrib.package;
        if (pkg === "swift") {
            iOSPlatformTag = platformElement;
        }
    });
    return iOSPlatformTag;
}

/* root is platform config element tree */
function hasPackageAttributeSwift (platformConfigEl) {
    let found = false;
    platformConfigEl.findall('platform').forEach(el => {
        if (el.attrib.package == 'swift') {
            found = true;
        }
    });
    return found;
...
module.exports.mergeSwiftPackage = mergeSwiftPackage;
module.exports.hasPackageAttributeSwift = hasPackageAttributeSwift;