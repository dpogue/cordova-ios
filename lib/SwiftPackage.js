...
function getPodSpecs (pluginEl) {
    return _getPodSpecsEl(pluginEl).map( ({type, tag}) => {
        ...
        const libraries = pods && pods.findall('pod')
            .filter(t => type === "normal" || (type === "swift" && _isTrue(t.attrib.nospm)))
            .map(t => t.attrib)
...
function _getPodSpecsEl (pluginEl) {
    const platform= 'ios';
    const swift = 'swift';
    const normal = "normal";
    const tag = 'podspec';
    return pluginEl.findall(tag).map(el => { return { type: normal, tag: el} }).concat(
        ...pluginEl.findall(`./platform[@name="${platform}"]`).map(el => {
        if (el.attrib.package === swift) {
            return el.findall('podspec').map(el => { return {type: swift, tag: el} });
        } else {
            return el.findall('podspec').map(el => { return {type: normal, tag: el} });
        }
    }));
}
...
module.exports.getPodSpecs = getPodSpecs;