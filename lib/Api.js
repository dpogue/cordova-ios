...
const { hasPackageAttributeSwift } = require('./SwiftPackage');
...
class Api {
    ...
    addPodSpecs (plugin, podSpecs, installOptions) {
        ...
        podSpecs.forEach(obj => {
            if (! hasPackageAttributeSwift(this.getPlatformInfo().projectConfig.doc.getroot())) {   
                // declarations
                if (obj.declarations) {
                ...
                if (obj.libraries) {
                    Object.keys(obj.libraries).forEach(key => {
                        ...
                        const isNoSpm = (
                            podJson.nospm === true ||
                            (typeof podJson.nospm === 'string' && podJson.nospm.toLowerCase() === 'true') ||
                            podJson.nospm === 1
                        );
                        if (!isNoSpm) {
